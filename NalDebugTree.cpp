...
QTreeWidgetItem* CNalParser::AddTreeItem(QTreeWidgetItem* rootItem)
{
    QTreeWidgetItem *newItem=0;
    if (NULL == rootItem) {
        newItem = new QTreeWidgetItem(m_pTree);
        newItem->setText(0, m_outputInfo);
        m_pTree->addTopLevelItem(newItem);
    }else{
...
void CNalParser::h264_debug_sps(sps_t* sps, QTreeWidgetItem* root)
{
    my_printf("seq_parameter_set_data()");
    QTreeWidgetItem* isps = AddTreeItem(root);
        ...
        sps->profile_idc == 122 || sps->profile_idc == 144 )
    {
        ...
        QTreeWidgetItem* ssmpf = AddTreeItem(isps);
    ...
    QTreeWidgetItem* fmof = AddTreeItem(isps);
    ...
    QTreeWidgetItem* fcf = AddTreeItem(isps);
    ...
    if (sps->vui_parameters_present_flag)
    {
        ...
        QTreeWidgetItem* oipf = AddTreeItem(vp);
        ...
        QTreeWidgetItem* vstpf = AddTreeItem(vp);
        if( sps->vui.video_signal_type_present_flag )
        {
            ...
            QTreeWidgetItem* cdpf = AddTreeItem(vstpf);
        ...
        QTreeWidgetItem* clipf = AddTreeItem(vp);
        ...
        QTreeWidgetItem* tipf = AddTreeItem(vp);
        ...
        QTreeWidgetItem* nhppf = AddTreeItem(vp);
        if( sps->vui.nal_hrd_parameters_present_flag )
        {
            my_printf("hrd_parameters()");
            QTreeWidgetItem* hp = AddTreeItem(nhppf);
        ...
        QTreeWidgetItem* vhppf = AddTreeItem(vp);
        if( sps->vui.vcl_hrd_parameters_present_flag )
        {
            my_printf("hrd_parameters()");
            QTreeWidgetItem* hp = AddTreeItem(vhppf);
            my_printf("cpb_cnt_minus1: %d  (v bits)", sps->hrd.cpb_cnt_minus1 );AddTreeItem(hp);
            my_printf("bit_rate_scale: %d  (4 bits)", sps->hrd.bit_rate_scale );AddTreeItem(hp);
            my_printf("cpb_size_scale: %d  (4 bits)", sps->hrd.cpb_size_scale );AddTreeItem(hp);
            int SchedSelIdx;
        ...
        QTreeWidgetItem* brf = AddTreeItem(vp);
...
void CNalParser::h264_debug_pps(pps_t* pps, QTreeWidgetItem* root)
{
    my_printf("pic_parameter_set_rbsp()");
    QTreeWidgetItem* ipps = AddTreeItem(root);
    ...
    if( pps->_more_rbsp_data_present )
    {
        ...
        QTreeWidgetItem* imrdp = AddTreeItem(ipps);
        ...
        QTreeWidgetItem* psmpf = AddTreeItem(imrdp);
...
void CNalParser::h264_debug_slice_header(h264_stream_t* h, QTreeWidgetItem* root)
{
    sps_t* sps = h->sps;
    pps_t* pps = h->pps;
    ...
    QTreeWidgetItem* slice = AddTreeItem(root);
    my_printf("slice_header()");
    QTreeWidgetItem* iheader = AddTreeItem(slice);
    ...
    QTreeWidgetItem* ppsid = AddTreeItem(iheader);
    ...
    if( !sps->frame_mbs_only_flag )
    {
        ...
        QTreeWidgetItem* fpf = AddTreeItem(iheader);
    ...
    if (nal->nal_unit_type == 20 || nal->nal_unit_type == 21)
    {
        ...
        QTreeWidgetItem* rplmm = AddTreeItem(iheader);
    }
    else
    {
        my_printf("ref_pic_list_modification()");
        QTreeWidgetItem* rplm = AddTreeItem(iheader);
        ...
        ( pps->weighted_bipred_idc == 1 && is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) ) )
    {
        ...
        QTreeWidgetItem* ipwt = AddTreeItem(iheader);
        ...
        for( int i = 0; i <= sh->num_ref_idx_l0_active_minus1; i++ )
        {
            ...
            QTreeWidgetItem* ilwl0f = AddTreeItem(ilwl0);
        ...
        for( int i = 0; i <= sh->num_ref_idx_l0_active_minus1; i++ )
        {
            if ( sps->ChromaArrayType != 0 )
            {
                ...
                QTreeWidgetItem* icwl0f = AddTreeItem(icwl0);
        ...
        if( is_slice_type( sh->slice_type, SH_SLICE_TYPE_B ) )
        {
            ...
            for( int i = 0; i <= sh->num_ref_idx_l1_active_minus1; i++ )
            {
                ...
                QTreeWidgetItem* ilwl1f = AddTreeItem(ilwl1);
            ...
            for( int i = 0; i <= sh->num_ref_idx_l1_active_minus1; i++ )
            {
                if ( sps->ChromaArrayType != 0 )
                {
                    ...
                    QTreeWidgetItem* icwl1f = AddTreeItem(icwl1);
    ...
    if( nal->nal_ref_idc != 0 )
    {
        ...
        QTreeWidgetItem* idrpm = AddTreeItem(iheader);
        ...
        else
        {
            ...
            QTreeWidgetItem* arpmmf = AddTreeItem(idrpm);
...
void CNalParser::h264_debug_aud(aud_t* aud, QTreeWidgetItem* root)
{
    my_printf("access_unit_delimiter_rbsp()");
    QTreeWidgetItem* iaud = AddTreeItem(root);
...
void CNalParser::h264_debug_seis( h264_stream_t* h, QTreeWidgetItem* root)
{
    sei_t** seis = h->seis;
    ...
    my_printf("sei_rbsp()");
    QTreeWidgetItem* isei = AddTreeItem(root);
    my_printf("sei_message()");
    QTreeWidgetItem* iisei = AddTreeItem(isei);
    ...
    for (i = 0; i < num_seis; i++)
    {
        ...
        QTreeWidgetItem* sp = AddTreeItem(iisei);
        ...
        case SEI_TYPE_USER_DATA_UNREGISTERED :
            {
                ...
                QTreeWidgetItem* udpb = AddTreeItem(sp);
                std::string sei_message;
                sei_message.append((char*)&s->payload[16], s->payloadSize-16);
                std::cout << "sei:" << sei_message << std::endl;
...
void CNalParser::h264_debug_nal_t(h264_stream_t* h, nal_t* nal)
{
    m_pTree->clear();
    my_printf("NAL");
    QTreeWidgetItem* root = AddTreeItem(0);
...
////////////////////////////////////////////////////////
void CNalParser::h265_debug_ptl(profile_tier_level_t* ptl, int profilePresentFlag, int max_sub_layers_minus1, QTreeWidgetItem* root)
{
...
void CNalParser::h265_debug_sub_layer_hrd_parameters(sub_layer_hrd_parameters_t* subhrd, int sub_pic_hrd_params_present_flag, int CpbCnt, int subLayerId, const char* p, QTreeWidgetItem* root)
{
    my_printf("[%s] sub_layer_hrd_parameters(%d)", p, subLayerId);
    QTreeWidgetItem* slhp = AddTreeItem(root);
...
void CNalParser::h265_debug_hrd_parameters(hrd_parameters_t* hrd, int commonInfPresentFlag, int maxNumSubLayersMinus1, QTreeWidgetItem* root)
{
    my_printf("hrd_parameters()");
    QTreeWidgetItem* ihrd = AddTreeItem(root);
...
// vps
void CNalParser::h265_debug_vps(h265_vps_t* vps, QTreeWidgetItem* root)
{
    int i, j;
    my_printf("video_parameter_set_rbsp()");
    QTreeWidgetItem* ivps = AddTreeItem(root);
    ...
    QTreeWidgetItem* iptl = AddTreeItem(ivps);
    ...
    {
        my_printf("SubLayers");
        QTreeWidgetItem* isub = AddTreeItem(ivps);
    ...
    QTreeWidgetItem* tipf = AddTreeItem(ivps);
    if (vps->vps_timing_info_present_flag)
    {
        ...
        QTreeWidgetItem* ppttf = AddTreeItem(tipf);
...
void CNalParser::h265_debug_scaling_list(scaling_list_data_t* sld, QTreeWidgetItem* root)
{
    my_printf("scaling_list_data()");
    QTreeWidgetItem* isl = AddTreeItem(root);
...
void CNalParser::h265_debug_short_term_ref_pic_set(h265_sps_t* sps, st_ref_pic_set_t*st, referencePictureSets_t* rps, int stRpsIdx, QTreeWidgetItem* root)
{
    my_printf("short_term_ref_pic_set( %d )", stRpsIdx);
    QTreeWidgetItem* srps = AddTreeItem(root);
    QTreeWidgetItem* srpsii = srps;
    if (stRpsIdx != 0)
    {
...
void CNalParser::h265_debug_vui_parameters(vui_parameters_t* vui, int maxNumSubLayersMinus1, QTreeWidgetItem* root)
{
    my_printf("vui_parameters()");
    QTreeWidgetItem* ivp = AddTreeItem(root);
    my_printf_flag("aspect_ratio_info_present_flag", vui->aspect_ratio_info_present_flag);
    QTreeWidgetItem* aripf = AddTreeItem(ivp);
    ...
    QTreeWidgetItem* oipf = AddTreeItem(ivp);
    ...
    QTreeWidgetItem* vstpf = AddTreeItem(ivp);
    ...
    QTreeWidgetItem* clipf = AddTreeItem(ivp);
    ...
    QTreeWidgetItem* ddwf = AddTreeItem(ivp);
    ...
    QTreeWidgetItem* vtipf = AddTreeItem(ivp);
    if (vui->vui_timing_info_present_flag)
    {
        ...
        QTreeWidgetItem* vppttf = AddTreeItem(vtipf);
        ...
        QTreeWidgetItem* vhppf = AddTreeItem(vtipf);
    ...
    QTreeWidgetItem* brf = AddTreeItem(vtipf);
...
// sps
void CNalParser::h265_debug_sps(h265_sps_t* sps, QTreeWidgetItem* root)
{
    ...
    // ptl
    my_printf("profile_tier_level()");
    QTreeWidgetItem* iptl = AddTreeItem(isps);
    ...
    QTreeWidgetItem* cwf = AddTreeItem(isps);
    ...
    my_printf("SubLayers");
    QTreeWidgetItem* isloripf = AddTreeItem(isps);
    ...
    QTreeWidgetItem* pef = AddTreeItem(isps);
    ...
    QTreeWidgetItem* ltrppf = AddTreeItem(isps);
    ...
    QTreeWidgetItem* sepf = AddTreeItem(isps);
    if (sps->sps_extension_present_flag)
    {
        my_printf_flag("sps_range_extension_flag", sps->sps_range_extension_flag);
        QTreeWidgetItem* sref = AddTreeItem(sepf);
        ...
        QTreeWidgetItem* smef = AddTreeItem(sepf);
...
// pps
void CNalParser::h265_debug_pps(h265_pps_t* pps, QTreeWidgetItem* root)
{
    my_printf("pic_parameter_set_rbsp()");
    QTreeWidgetItem* ipps = AddTreeItem(root);
    ...
    QTreeWidgetItem* cqdef = AddTreeItem(ipps);
    ...
    QTreeWidgetItem* tef = AddTreeItem(ipps);
    if (pps->tiles_enabled_flag)
    {
        ...
        QTreeWidgetItem* usf = AddTreeItem(tef);
    ...
    QTreeWidgetItem* dfcpf = AddTreeItem(ipps);
    if (pps->deblocking_filter_control_present_flag)
    {
        ...
        QTreeWidgetItem* dfdf = AddTreeItem(dfcpf);
    ...
    QTreeWidgetItem* sldf = AddTreeItem(ipps);
...
// aud
void CNalParser::h265_debug_aud(h265_aud_t* aud, QTreeWidgetItem* root)
{
...
// sei
void CNalParser::h265_debug_seis(h265_stream_t* h, QTreeWidgetItem* root)
{
...
void CNalParser::h265_debug_ref_pic_lists_modification(h265_slice_header_t* hrd, QTreeWidgetItem* root)
{
...
void CNalParser::h265_debug_pred_weight_table(h265_stream_t* h, QTreeWidgetItem* root)
{
    pred_weight_table_t* pwt = &h->sh->pred_weight_table;
    h265_sps_t* sps = h->sps;

    my_printf("pred_weight_table()");
    QTreeWidgetItem* ipwt = AddTreeItem(root);
...
void CNalParser::h265_debug_slice_header(h265_stream_t* h, QTreeWidgetItem* root)
{
    ...
    QTreeWidgetItem* islce = AddTreeItem(root);

    my_printf("slice_segment_header()");
    QTreeWidgetItem* ihrd = AddTreeItem(islce);
    ...
    if (!hrd->dependent_slice_segment_flag)
    {
        ...
        QTreeWidgetItem* dssf = AddTreeItem(ihrd);
        ...
        if (hrd->slice_type == H265_SH_SLICE_TYPE_P || hrd->slice_type == H265_SH_SLICE_TYPE_B)
        {
            ...
            QTreeWidgetItem* nriaof = AddTreeItem(dssf); // ����H265VideoESViewer��û�нڵ�
        ...
        if (hrd->deblocking_filter_override_flag)
        {
            ...
            QTreeWidgetItem* dfof = AddTreeItem(dssf);
    ...
    if (pps->tiles_enabled_flag || pps->entropy_coding_sync_enabled_flag)
    {
        ...
        QTreeWidgetItem* inepo = AddTreeItem(ihrd);
        if (hrd->num_entry_point_offsets > 0)
        {
            ...
            QTreeWidgetItem* iinepo = AddTreeItem(ihrd);
...
void CNalParser::h265_debug_nal_t(h265_stream_t* h, h265_nal_t* nal)
{
    ...
    QTreeWidgetItem* subroot = AddTreeItem(root);
